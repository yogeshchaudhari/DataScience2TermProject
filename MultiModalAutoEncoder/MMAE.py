...
class MMAE():
    def __init__(self):
        self.name = 'MMAE'
        self.K = 10
        self.D1 = "Combined_rna_cna_geneexp"
        self.D2 = 'CLINICAl-25'
        self.alpha = 0.4
        self.beta = 0.1
...
dnn_md1 = MMAE()
dnn_md1.load_config()
d_matrix, d_class = dnn_md1.load_txt(dnn_md1.D1, 1601)
dnn_md1.epoch = 40
...
dnn_md2 = MMAE()
dnn_md2.load_config()
d_matrix, d_class = dnn_md2.load_txt(dnn_md2.D2, 19)
dnn_md2.epoch = 60
dnn_md2.MAX_STEPS = [dnn_md2.epoch, dnn_md2.epoch, dnn_md2.epoch, dnn_md2.epoch, dnn_md2.epoch, dnn_md2.epoch,
                     dnn_md2.epoch, dnn_md2.epoch, dnn_md2.epoch, dnn_md2.epoch]  # 3000,3000,3000,100 CLINICAL-25
dnn_md2.hidden_units = [1000, 1000, 1000, 100]  # 1000 1000 1000 100
# dnn_md2.active_fun = 'relu'
dnn_md2.BATCH_SIZE = 128
dnn_md2.END_LEARNING_RATE = 0.001
dnn_md2.drop = 1.0
dnn_md2.IS_PRINT_INFO = "F"
label1, cls = dnn_md2.code_lables(d_class, dnn_md2.MT_CLASS_TASK1)
class_predict_fcn2, p_valid_all2, cls_valid_all2 = dnn_md2.train(kf1, dnn_md2.scale_max_min(d_matrix), d_class, cls, ut)

dnn_md1.alpha = 0.3
dnn_md1.beta = 1 - dnn_md1.alpha
dnn_md2.gamma = 1-dnn_md1.alpha-dnn_md1.beta

# validation
p_valid_all = dnn_md1.alpha * p_valid_all1 + dnn_md1.gamma * p_valid_all2
...
print(
    "DNN-validation## ACC: %s,AUC %s,PRE %s,REC %s,F1 %s, PR_AUC %s" % (acc_f, auc_fcn, pre_f, rec_f, f1_f, pr_auc_fcn))

# test
class_predict_fcn = dnn_md1.alpha * class_predict_fcn1 + dnn_md1.gamma * class_predict_fcn2
...
name = dnn_md1.name + '_' + str(dnn_md1.hidden_units[0]) + '-' + str(dnn_md1.hidden_units[1]) + '-' + str(
    dnn_md1.hidden_units[2]) + '-' + str(dnn_md1.hidden_units[3]) + '_' + str(dnn_md1.alpha) +'_'+str(dnn_md1.beta)+'_'+str(dnn_md1.gamma)
# SCORE
